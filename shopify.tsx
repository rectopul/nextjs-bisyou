...
import { DataProductWithMedia, Media } from "./@types/shopify/ProductWithMedia"
...
async function ShopifyAdminData(
  ...
  const options: RequestInit = {
    ...
    headers: {
      ...
      "X-Shopify-Access-Token": process.env.SHOPIFY_API_ADMIN_ACESS_TOKEN || "",
    },
...
export async function productsByTermAdmin(
  ...
  try {
    const productsByTerm: Shopify.ProductsByTerm = await ShopifyAdminData(query)
...
export async function listCategories(): Promise<Shopify.ProductCategory[]> {
  ...
  try {
    const productCategories: ProductCategoryEdge = await ShopifyAdminData(query)
        ...
        if (
          ...
          if (
            verificarValorNaString(
              filters,
              p.node.field_nescessidades_do_produto.reference.field.value,
            ) &&
...
export async function getMetaObject(type: string) {
  ...
  try {
    const response: Shopify.MetaObjects.MetaObjectData =
...
export async function createCart(): Promise<Shopify.Cart.Create | null> {
  ...
  try {
    const data: Shopify.Cart.Create = await ShopifyData(query)
